...
const useMedia = (loadItems = false) => {
  ...
  const editMedia = async (media_id, inputs, token) => {
    const fetchOptions = {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
  ...
  return {mediaArray, postMedia, deleteMedia, editMedia};
};

const useLike = (media_id) => {
  const [likeCount, setLikeCount] = useState(0);

  const getLikeCountByMediaId = useCallback(
    async () =>
      await fetchData(
  ...
  useEffect(() => {
    const getCounts = async () => {
      const likeCount = await getLikeCountByMediaId();
      if (likeCount?.count) {
